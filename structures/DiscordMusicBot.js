    ...
    this.Manager = new Manager({
      ...
      .on("queueEnd", (player) => {
        ...
        } else
        {
          ...
          listQuit = listQuit.concat(q);
          console.log(listQuit);
          if(!quit){
            quit = true;
          }
          async function f1() {
            await new Promise(resolve => setTimeout(function() { // Setup a timer
              if(quit){
              player.destroy();
              let embed = new MessageEmbed()
              .setTitle(":notes: | **Disconnected!**");
